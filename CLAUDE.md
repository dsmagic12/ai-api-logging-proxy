...
The proxy is a **transparent provider gateway** built on Fastify. Requests flow in provider-native shape, are forwarded upstream via `undici`, and streamed back to the client without buffering the full response. A side-channel observer parses each chunk for logging only.
...
3. After the stream ends (or on error), one audit record is written by `AuditLogger`.
...
`AuditLogger` writes `logs/ai-api-YYYY-MM-DD.jsonl`. `ToolEventLogger` writes `logs/ai-tool-events-YYYY-MM-DD.jsonl`. Both call `redactJson` before writing; neither buffers records in memory. The log file path is computed at construction time (process start), so date rollover requires a restart in long-running processes.
...
**Route auth**: A `preHandler` hook calls `proxyAuth` on all routes except `/`, `/public/*`, `/demo/*`, `/tooling/config`, and `/healthz`. Auth is skipped entirely when `PROXY_SHARED_SECRET` is empty.
...
**Two log streams are separate**: Provider request logs (`AuditRecord`) and tool event logs (`ToolEventRecord`) are written to different files and have different schemas. Do not conflate them.
...
GET  /demo/logs                Demo log records (no auth)
