...
export function detectDependencyLoop(record) {
    if (record.event_type !== 'command' && record.event_type !== 'tool_use')
        return [];
...
function categorizeCommand(command) {
    ...
    if (/\b(ps|pgrep|jobs|lsof)\b.*\b(npm|node|install)\b/.test(normalized) ||
...
function buildSignals(observations, currentCommand, currentTimestampMs) {
    ...
    const pollingCount = count(recent.filter((item) => currentTimestampMs - item.timestampMs <= 5 * 60 * 1000), 'polling_check');
    const signals = [];
    if (nodeModulesDeleteCount >= 2 && lockfileDeleteCount >= 2 && npmInstallCount >= 2) {
        signals.push({
            code: 'npm_dependency_reinstall_loop',
            severity: 'critical',
            message: 'Repeated node_modules/package-lock deletion followed by npm install detected in the same tool session.',
            evidence: {
                window_minutes: 30,
            ...
            recommendation: 'Stop reinstalling dependencies. Inspect the first npm error, compare package manager/version, check package.json constraints, and try npm ci or targeted dependency updates instead.'
        });
    }
        ...
        signals.push({
            ...
            recommendation: 'Avoid repeating npm install without changing the cause. Capture the install error once, summarize it, and choose a targeted remediation.'
        });
    }
    if (lockfileDeleteCount >= 3) {
        signals.push({
            ...
            recommendation: 'Treat lockfile deletion as an explicit last resort. Prefer npm ci for reproducible installs or update the lockfile once after resolving package constraints.'
        });
    }
    if (pollingCount >= 5) {
        signals.push({
            ...
            recommendation: 'Use a longer wait interval, stream the process output once, or wait for command completion instead of repeatedly asking the model to check process status.'
        });
    }
    return signals;
}
function count(observations, category) {
