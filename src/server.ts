...
import {
  anthropicPathFromWildcard,
  ensureProviderConfigured,
  openAIPathFromWildcard,
...
app.get('/demo/logs', async () => ({
  records: [
    ...
    },
    {
      ...
      coaching_signal:
        'Critical: repeated deletion of node_modules and package-lock.json followed by npm install. Stop reinstalling and inspect the first npm error.'
    }
  ]
}));

app.post('/tool-events', async (request, reply) => {
  const body = (request.body ?? {}) as Partial<ToolEventRecord> & Record<string, unknown>;
  const record: ToolEventRecord = {
    ...
    tool:
      body.tool === 'claude-code' || body.tool === 'codex'
        ? body.tool
        : 'unknown',
    ...
    prompt_summary:
      body.prompt_summary ?? (body.prompt === undefined ? undefined : summarizeRequestBody(body.prompt)),
    response_summary:
      body.response_summary ?? (body.response === undefined ? undefined : redactJson(body.response)),
    usage: body.usage as ToolEventRecord['usage'],
    metadata: redactJson({
      source: body.source,
...
app.post('/demo/dependency-loop', async (request, reply) => {
  const body = (request.body ?? {}) as { tool?: 'claude-code' | 'codex'; session_id?: string };
  ...
  const results: Array<{ command: string; coaching_signals: ToolEventRecord['coaching_signals'] }> = [];
...
app.get('/demo/stream', async (request, reply) => {
  const provider = String((request.query as { provider?: string }).provider ?? 'openai');
...
function normalizeToolEventType(value: unknown): ToolEventRecord['event_type'] {
  ...
  return typeof value === 'string' && allowed.has(value)
    ? (value as ToolEventRecord['event_type'])
    : 'heartbeat';
}
