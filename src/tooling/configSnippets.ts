export function codexConfig(proxyBaseUrl: string): string {
...
export function claudeCodeSettings(proxyBaseUrl: string): string {
  const normalized = proxyBaseUrl.replace(/\/$/, '');
  return `{
  "env": {
...
export function shellWrappers(proxyBaseUrl: string): string {
  const normalized = proxyBaseUrl.replace(/\/$/, '');
